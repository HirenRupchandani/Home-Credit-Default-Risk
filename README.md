...
This dataset consists of information about applicant’s previous loans in Home credit. It contains attributes such as the type of loan, the status of the loan, the down payment, etc.
**instalments_payments.csv**
This dataset contains information about the repayment of previous loans.
...
**bureau.csv**
This dataset consists of information on the individual’s previous credit history at other financial institutions that were reported to the credit bureau.
...
![data leakage](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/DataLeakage.png?raw=true)
  ...
  - Among these two, X_valid was used as an evaluation set, which may cause some leakage between X_valid and X_train features, but ensured there is no leakage between X_test and X_train datasets, and subsequently no leakage between X_kaggle_test and X_train datasets.
  - There might be some information leakage from the X_train set over to the other datasets due to imputation and PCA operations but that is as far as the data leakage goes.

- As for **cardinal sins of machine learning**, we are confident to check that we have not made such mistakes and assumptions on the data that might affect the EDA, interpretation, or the modeling and predictions of the plethora of models that we have implemented.

## Pipelines

- Just like the last phase, creating separate pipelines will help us to fill in missing values and perform scaling on numerical features.  
...
![Pipeline.jpeg](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/15c28149133555ae5932a10f193ca38db8aabd30/Images/Pipeline.jpeg?raw=true)

- Additionally, to avoid imbalance class issues, we have undersampled the data (using the RandomUnderSampler class) in this phase to make it balanced.

![rr2.png](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/rr2.png?raw=true)

- The target feature exhibits the following distribution:

![donut_distribution_target.jpg](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/donut_distribution_target.jpg?raw=true)
...
- The RandomForestClassifier class has been used to implement the Random Forest algorithm for feature importance. After training the model, we used the feature_importances_ property to retrieve the relative importance scores for each input feature.

![rr1.png](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/rr1.png?raw=true)

- Some of the important features are as follows:

![FeaturePlot.png](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/FeaturePlot.png?raw=true)
...
![performance_other_models.jpeg](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/performance_other_models.jpeg?raw=true)

- Moving a step further, we created various MLPs (more than 15) and these are the top 5 models based on the best Test F1 score:

![Sigmoid_Relu.jpeg](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/Sigmoid_Relu.jpeg?raw=true)
...
![Epoch-plot.png](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/Epoch-plot.png?raw=true)

- After selecting the best model, we see that our best-performing model looks like this:

![nn (1).png](https://github.com/HirenRupchandani/Home-Credit-Default-Risk/blob/main/Images/nn%20(1).png?raw=true)
...
- This phase of the project primarily focused on two things:
  ...
  - Prepare a Multi-Layer Perceptron using the PyTorch library and select the best set of hyperparameters.
...
- RandomUnderSampler and RandomForestClassifier's feature_importances_ enable us to achieve these with ease.
